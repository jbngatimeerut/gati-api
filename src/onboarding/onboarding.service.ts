...
export class OnboardingService {
  ...
  async accept(t: string, dto: { password: string; phone?: string; company?: string; category?: string }) {
    ...
    this.embeddings.reindexMember(member.id).catch(() => {});
    if (inv.invitedById) {
      this.notifications.notify(inv.invitedById, {
        type: 'INVITE_ACCEPTED',
        title: `${member.name} accepted your invite`,
        body: `Joined as ${inv.roleKey.replace(/_/g, ' ')}`,
        entityType: 'Member',
        entityId: member.id,
      }).catch(() => {});
    }
