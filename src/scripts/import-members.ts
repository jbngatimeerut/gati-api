...
function tempPwd() { return 'Gati@' + Math.floor(1000 + Math.random() * 9000) + Math.random().toString(36).slice(2, 4); }
function cardToken() { const a = 'ABCDEFGHJKMNPQRSTUVWXYZ23456789'; let t = ''; for (let i = 0; i < 10; i++) t += a[Math.floor(Math.random() * a.length)]; return `GATI-${t.slice(0, 5)}-${t.slice(5)}`; }
